...
fn build_model_and_tokenizer(model_id: String, revision: String, use_pth: bool) -> Result<(BertModel, Tokenizer, usize, usize)> {
...
fn get_max_input_length(contents: &str) -> Result<usize> {
  ...
  let max_length = config["max_position_embeddings"].as_u64().ok_or_else(|| {
    std::io::Error::new(std::io::ErrorKind::Other, "Max position embeddings not found")
  })?;
...
fn get_hidden_size(contents: &str) -> Result<usize> {
  let config: Value = serde_json::from_str(&contents)?;
  let max_length = config["hidden_size"].as_u64().ok_or_else(|| {
    std::io::Error::new(std::io::ErrorKind::Other, "Hidden size not found")
  })?;
  Ok(max_length as usize)
}
...
impl Model {
  ...
  pub fn create(model_id: String, revision: Option<String>, use_pth: Option<bool>) -> Self {
  	...
  	let (model, tokenizer, max_input_len, hidden_size) = build_model_and_tokenizer(model_id, revision, use_pth).unwrap();
    Model {
    	model,
    	tokenizer,
      max_input_len,
  ...
  /// @return array<string>
	pub fn predict(&mut self, text: String) -> Vec<f32> {
		...
		truncation.max_length = self.max_input_len;
